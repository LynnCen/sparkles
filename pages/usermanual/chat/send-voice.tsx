...
import step1 from "../../../public/images/user/chat-7.jpg";
import TDK from "../../../components/TDK/TDK";
const A: NextPage = (props) => {
  const { t } = useTranslation("user");
  const data = [
    {
      title: t("chat-send-voice-descript1"),
      src: step1,
    },
    {
      title: t("chat-send-voice-descript2"),
    },
  ];
  return (
    <>
      <TDK title={`${t("chat-send-voice")} | ${t("user-mannual")}`} />
      <h2>{t("chat-send-voice")}</h2>
      {data.map((item, index) => {
