...
import { queryList as queryCategory, CateType } from '@/pages/MiniCategory/service';
...
const TableList: React.FC<PropsType> = (props) => {
  ...
  useEffect(() => {
    queryCategory({ page: 1, row: 1000 }).then(({ data }) => setAllCategory(data), console.log);
  ...
  const columns: ProColumns<ItemType>[] = [
    ...
    {
      title: formatMessage({ id: 'table-link-url' }),
      dataIndex: 'link_url',
      hideInSearch: true,
      width: 100,
      ellipsis: true,
    ...
    },
    {
      ...
      ellipsis: true,
      render: (_, record) => {
        if (!record.cate_id) return '-';
        const cate = allCategory.find((category) => category._id === record.cate_id);
