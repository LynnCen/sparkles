...
const TableList: React.FC<PropsType> = (props) => {
  ...
  useEffect(() => {
    // @ts-ignore
    setAuthObj(getPermissionsAsObj(props.user.currentUser.access, ['setting', 'jump']));
  }, [props.user]);
  const actionRef = useRef<ActionType>();
  const columns: ProColumns<ItemType>[] = [
    {
      title: formatMessage({ id: 'table-linkID' }),
      dataIndex: '_id',
      hideInSearch: true,
    },
    {
    ...
    },
    {
      ...
      hideInSearch: true,
      render(_, record) {
        if (record.type === 2) return formatMessage({ id: 'table-whitelist' });
        if (record.type === 3) return formatMessage({ id: 'table-blacklist' });
        return record.type;
      },
    },
    {
