...
const TableList: React.FC<PropsType> = (props) => {
  ...
  useEffect(() => {
    // @ts-ignore
    setAuthObj(getPermissionsAsObj(props.user.currentUser.access, ['setting', 'copy']));
  }, [props.user]);

  useEffect(() => {
    getLangList().then(
      (data) => setLangObj(data),
      (e) => console.log(e),
    );
  }, []);
  ...
  const columns: ProColumns<ItemType>[] = [
    ...
    },
    {
      ...
      filters: [],
      valueEnum: Object.keys(langObj)
        .map((lang) => {
          return { [lang]: { text: langObj[lang], lang } };
        })
        .reduce((acc, cur) => ({ ...acc, ...cur }), {}),
    },
    {
      title: formatMessage({ id: 'table-key' }),
      dataIndex: 'key',
      width: 260,
      ellipsis: true,
    },
