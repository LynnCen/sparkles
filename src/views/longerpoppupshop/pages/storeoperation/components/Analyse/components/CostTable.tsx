...
const CostTable: FC<any> = ({ params, setVisible, setContent, tenantStatus }) => {
  ...
  const columns = [
    ...
    { title: '租金（元）', key: 'rent', width: 80 },
    {
      ...
      render: (text, record) => (
        <div className='ct'>
          <IconFont iconHref='icona-ic_shouyeshipin' className='fs-24 pointer' onClick={() => jumpMonitoring(record.monitoringId)}></IconFont>
        </div>
      ),
    },
  ];
