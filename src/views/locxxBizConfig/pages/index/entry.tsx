...
const Dictionary: React.FC = () => {
  ...
  const [groupEncode, setGroupEncode] = useState<string>('');
  ...
  const { loadData, onSelectTree } = useMethods({
    ...
    loadData: async () => {
      ...
      const defaultTargetEncode = objectList[0] && objectList[0].encode;
      setDictionaryData(objectList);
      setManagePermissions(permissions);
      defaultTargetId && setDictionaryId(defaultTargetId);
      defaultTargetEncode && setGroupEncode(defaultTargetEncode);
    },
    // 选择左侧树状数据时，获取对应的id
    onSelectTree: (id: number, encode: string) => {
      setDictionaryId(id);
      setGroupEncode(encode);
    }
  });
    ...
    <KeepAlive saveScrollPosition>
      <Layout className={styles.dictionaryPage}>
        ...
        <Layout className='ml-12'>
          {/* 右侧表格页 */}
          <Content>
            <Core groupId={dictionaryId} groupEncode={groupEncode}/>
          </Content>
        </Layout>
      </Layout>
    </KeepAlive>
