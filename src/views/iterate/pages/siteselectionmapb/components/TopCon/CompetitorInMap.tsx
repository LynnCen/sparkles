...
const CompetitorInMap: FC<any> = ({
  ...
  useEffect(() => {
    // 使用测距时
    if (isStadiometry) {
      massMarkerRef.current && massMarkerRef.current.hide();
      return;
    }
    massMarkerRef.current && massMarkerRef.current.show();
  }, [isStadiometry]);
  ...
  const loadData = async () => {
    ...
    if (isArray(points) && points.length) {
      ...
      points.forEach((item: any) => {
        const { brandId, optimizeLogo, lng, lat } = item;
        const itemData: any = {
          ...item,
          lnglat: [+lng, +lat]
        };
      ...
      if (isStadiometryRef.current) { // 异步的缘故
        massMarkerRef.current.hide();
      }
