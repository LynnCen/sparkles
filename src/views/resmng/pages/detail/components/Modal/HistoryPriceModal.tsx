import { brandSearch } from '@/common/api/brand';
...
const HistoryPriceModal: React.FC<HistoryPriceModalProps> = ({
  ...
  const { parseTitle, onCancel, loadBrandData, loadIndustryData } = useMethods({
    ...
    loadBrandData: async () => {
      const result = await brandSearch({ type: 1, brandId: historyPriceModalInfo.brandId });
      if (result) {
        const resultOptions = result.map((item) => {
  ...
  const searchBrand = async (name) => {
    const result = await brandSearch({ type: 1, name, brandId: historyPriceModalInfo.brandId });
    if (result) {
      const resultOptions = result.map((item) => {
        return { label: item.name, value: item.id };
      });
  ...
  const onSearch = (value) => {
    const newFn = debounce(searchBrand, 500);
    newFn(value);
  };
    ...
    <Modal title={parseTitle()} open={historyPriceModalInfo.visible} onOk={onSubmit} onCancel={onCancel}>
      <Form {...layout} form={form}>
        <FormSelect
          ...
          config={{ showSearch: true, onSearch, filterOption: false }}
          formItemConfig={{
