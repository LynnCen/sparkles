...
const Map: FC<any> = ({
  ...
  // 点位、门店总数
  const DataCount:FC<any> = () => <div className={styles.dataCount}>
    <span className={styles.dataCountContent}>
      <span className={styles.dataCountLabel}>在库点位数：</span>
      <span className={styles.dataCountValue}>{nationalDate?.chancePointAllCount || '-'}</span>
      <span className='mr-10'>个</span>
    </span>
    <span className={styles.dataCountContent}>
      <span className={styles.dataCountLabel}>在营业门店：</span>
      <span className={styles.dataCountValue}>{nationalDate?.operationOpenCount || '-'}</span>
      <span>个</span>
    </span>
  </div>;
