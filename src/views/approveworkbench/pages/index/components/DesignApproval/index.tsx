...
import { saveShopDesignAdvance, createApprovalShop } from '@/common/api/approveworkbench';
...
const DesignApproval: FC<DesignApprovalProps> = ({
  open,
  setOpen,
  onRefresh,
}) => {
  ...
  const {
    ...
    dealSaveParams(formData) {
      let materialUrls;
      if (Array.isArray(formData.materialUrls)) {
        materialUrls = formData.materialUrls.map((itm: any) => itm.url);
      }
      const params = {
        ...formData,
        materialUrls,
        shopEvaluationId: spot.id,
        id: spot.designAdvanceId,
      };
    ...
    async confirmHandle() {
      ...
      saveShopDesignAdvance(params).then(() => {
        createApprovalShop({
          evaluationId: spot.id,
          typeValue: 9, // 申请类型（8点位申请、9设计申请、10合同申请）
        }).then(() => {
  ...
  return (
    <V2Drawer open={open} onClose={closeDrawer} className={styles.formDrawer} destroyOnClose>
      <V2Container
        // 24px上padding
        style={{ height: 'calc(100vh - 24px)' }}
        extraContent={{
          top: <V2Title text='提前设计审批'/>,
      ...
      >
        <SelectSpot open={open} selectType={SelectSpotType.DESINGN} setSpotDetail={setSpot}/>
        {spot.id ? <DesignForm form={form} spot={spot}/> : <></>}
      </V2Container>
    </V2Drawer>
  );
};
