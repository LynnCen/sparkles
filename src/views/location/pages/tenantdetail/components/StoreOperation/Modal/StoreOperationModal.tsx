...
import { overviewImport } from '@/common/api/location';
...
const StoreOperationModal: FC<any> = ({
  ...
  const download = () => {
    downloadFile({
      name: '门店导入模版',
      url: `https://staticres.linhuiba.com/project-custom/custom-flow/file/门店运营导入模版.xlsx`
    });
  };
  ...
  const submitHandle = () => {
    form.validateFields().then(async (values: FormData) => {
      ...
      const param = { url, urlName: name, tenantId };
      setIsLock(true);
      setLoading(true);
      await overviewImport(param).then(() => {
        confirmHandle();
        ...
        <div className={styles.importExcel}>
          ...
          <V2Form form={form} name='form' colon={false}>
            ...
            <div className={styles.template}>
              ...
              <span className='ml-3'>门店运营导入模版.xlsx</span>
