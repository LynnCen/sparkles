...
import { getChancePointStatusConfigList } from '@/common/api/location';
...
const ChancePointConfig: FC<Props> = ({
  tenantId,
  mainHeight,
}) => {
  ...
  {
    key: 'anotherName', title: '重命名', dragChecked: true
  },
  {
    title: '操作',
    key: 'permissions',
  ...
  const loadData = async() => {
    const data = await getChancePointStatusConfigList({ tenantId });
    return {
      dataSource: data,
      count: data.length,
    };
...
export default ChancePointConfig;
