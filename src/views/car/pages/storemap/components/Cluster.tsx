...
const Cluster: FC<{
  ...
  const {
    ...
    switchLevel: async () => {
      ...
      switch (level) {
        case COUNTRY_LEVEL:
          const provinceRes = await storeCount({
          ...
          break;
        case PROVINCE_LEVEL:
          const cityRes = await storeCount({
          ...
          if (labelMarkerRef.current) {
            ...
            cluster.data?.forEach(item => {
              labelContent += `<div class='item'>
                ${true ? (`<div>
                                  <img src=${item.image && item.image !== '' ? item.image : 'https://staticres.linhuiba.com/project-custom/locationpc/img_placeholder.png'} />
                              </div>`) : ''}
                <div>
            ...
            labelMarkerRef.current.setOffset(new window.AMap.Pixel(-34, 38));
          }
