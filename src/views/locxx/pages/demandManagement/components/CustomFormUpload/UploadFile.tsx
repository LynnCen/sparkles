 ...
 * @LastEditTime 2024-01-11 11:01
 * @FilePath /saas-manage/src/views/locxx/pages/demandManagement/components/CustomFormUpload/UploadFile.tsx
 * @Description 业务上传文件组件
 */
/* version: 2.15.4 */
...
import { antPrefix, officeView, videoTypes, imageTypes, pdfView, typeMap, specialDownloadTypes } from '@/common/components/config-v2';
...
import { CombineUploadProps } from '@/common/components/Form/V2FormUpload/props';
...
const V2UploadFile: React.FC<V2UploadFileProps> = ({
  ...
  const methods = useMethods({
    ...
    async customRequest({
      ...
      const config = {
        headers: { ...headers, 'Content-Type': 'multipart/form-data', clientKey: 1000210, token: getCookie('kunlun_token') },
      ...
      /**
       * 后端上传七牛云
       * https://yapi.lanhanba.com/project/307/interface/api/68011
       */
      axios.post('/lcn-api/api/locxx/requirement/uploadFile', formData, config)
        .then(({ data }) => {
          onSuccess(data, file);
        })
        .catch(onError);
    ...
    onChange(file: any) {
      ...
      if (file.file.status === 'done') { // 上传成功
        ...
        const arr = file.fileList.map((item: any) => {
          ...
          if (item.url || item.response?.url) {
            newItem.url = item.url || item.response?.url;
            if (item.response?.originalUrl) {
              newItem.originalUrl = item.response?.originalUrl;
            }
          }
          return newItem;
        });
