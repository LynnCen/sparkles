...
import { Avatar, Modal, Tooltip, Checkbox } from 'antd';
import { dispatchNavigate } from '@/common/document-event/dispatch';
// import ModalForm from './ModalForm';
// import ModalAssigned from './ModalAssigned';
import { areaIcon, leftIcon, rankOptions, rankStatus } from '../ts-config';
...
const Result: FC<any> = ({
  ...
  useEffect(() => {
    ...
    if (!visible) {
      // areaData[0] && methods.selectArea(areaData[0], 0);
      protectedAreaGroup.show();
    } else {
      // const ind = areaData.findIndex(val => val.brandMark === 0);
      // if (ind !== -1) {
      //   methods.selectArea(areaData[ind], ind);
      // } else {
      //   areaLabelRef.current.setContent('   '); // 如果所有的区域都需要隐藏，手动把文字显示隐藏
      // };
  ...
  const methods = useMethods({
    ...
    getReport: async () => {
      const { objectList } = await get('/shop/report/list', { status: 1, page: 1, size: 10 }, {
        isMock: false,
        mockId: 331,
        mockSuffix: '/api',
        needHint: true
      });
      setReportList(objectList);
    },
    handleDetail(record: any) {
      ...
      if (modelName === '自定义指标推荐') {
        ...
        navigate(`/recommend/reportdetail?id=${id}`, { replace: true });
    ...
    selectArea(val, ind) {
      ...
      if (areaData[showInd]) {
        ...
        // areaData[showInd]?.areaMarker?.setIcon(
        //   new window.AMap.Icon(methods.getAreaIconOpt(showInd))
        // );
        if (showInd === 0) {
      ...
      // );
      // _mapIns.setCenter([val.lng, val.lat], false, 100);
      // _mapIns.setZoom(17, false, 100);
      if (val?.lng && val?.lat) {
    ...
    getAreaIconOpt: (ind, select) => {
      let image = '';
      if (select) {
        image = ind < 3 ? areaIcon[ind + 3].url : '//staticres.linhuiba.com/project-custom/locationpc/lantu/ic_derdingwei_select.png';
      } else {
        image = ind < 3 ? areaIcon[ind].url : '//staticres.linhuiba.com/project-custom/locationpc/lantu/ic_defdingwei.png';
      }
      return {
        image,
        size: [48, 48],
    ...
    <>
      <MapDrawer
        ...
        mapDrawerStyle={{
          ...
          transform:
          leftDrawerVisible ? 'translateX(0%)' : 'translateX(-326px)'
        }}
      ...
      >
        <div className={styles.leftCon} >
          <div className={styles.title}>
            ...
            <div className={styles.pModel}>{model.modelName}推荐结果</div>
            {/* 这个创建拓店任务是历史逻辑了，现在已经和app的创建拓店任务不是一个逻辑了，凯斌和产品团威已经确认过了，接口会下掉，前端注释掉 */}
            {/* <div>
            <Button
              type='primary'
              block={true}
              onClick={methods.createHandle}
            >创建拓店任务</Button>
          </div> */}
            {
              !isCoffee && (<div className={styles.check}>
                <Checkbox
                  checked={visible}
                  onChange={(e) => {
          ...
          {/* 产品团威要求所有租户隐藏该入口 */}
          {/* { isCoffee ? null : (
          <a onClick={() => dispatchNavigate(`/recommend/modelreport?id=${id}`)}>
            查看完整报告 {'>'}
          </a>
        )} */}
          {/* <ModalForm
      ...
      <MapDrawer
        ...
        mapDrawerStyle={{
          ...
          maxHeight: 'calc(100vh - 70px)', // 动态高度，70是根据UI稿
          transform:
            rightDrawerVisible ? 'translateX(0%)' : 'translateX(290px)'
        }}
      ...
      >
        {reportList.map(report => <div
          className='report'
          onClick={() => methods.handleDetail(report)}
          key={report.id}>
          <div className='label'>
            {report.areaName}
            {report.modelName}
          ...
          <div
            className='icon'>
            <IconFont iconHref='iconarrow-right' />
          </div>
        </div>)}
      </Modal>
    </>
