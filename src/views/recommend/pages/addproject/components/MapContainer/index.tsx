...
import { NOT_MORE_DATA, RankStatus, addBusiness, mapCon, markerType, maxAddressMarkerCount } from './ts-config';
// import { getSession } from '@lhb/cache';
import { useAmapLevelAndCityNew } from '@/common/hook/useAmapLevelAndCityNew';
import { getAreaMapData, getAreaPaging, getCapacityArea, getMapStatisticsInfo, getPolygon, getTreeSelection, getSelection } from '@/common/api/networkplan';
import { BUSINESS_ZOOM, CITY_LEVEL, DISTRICT_LEVEL, DISTRICT_ZOOM, PROVINCE_LEVEL } from '@/common/components/AMap/ts-config';
...
const MapContainer:FC<any> = ({
  ...
  // const planManagementDetail = getSession('planManagementDetail');
  ...
  const [isShowBusinessDistrict, setIsShowBusinessDistrict] = useState<boolean>(false);// 是否展示商区围栏，此抽屉地图页默认false，
  ...
  const curClickTypeRef = useRef<markerType>(curSelectedBusiness?.planClusterId ? markerType.AddressMarker : null);
  ...
  const getRightListData = async(initData = false) => {
    ...
    if (curSelectedBusiness?.planClusterId && !searchParams?.sortField) {
      params = { ...params, orderByIds: [curSelectedBusiness?.planClusterId] };
    }
    setLoading(true);
  ...
  useDebounceEffect(() => {
    ...
    const commonParams = {
      parentCompanyPlanned: parentCompanyPlanned,
      childCompanyPlanned: childCompanyPlanned,
  ...
  }, [amapIns, district?.id, level, businessTypeList, searchParams, isReset, bounds, isBusinessZoom, curSelectedBusiness?.planClusterId], 100);
  ...
  // useEffect(() => {
  //   if (!amapIns) return;
  //   if (curSelectedBusiness?.lng && curSelectedBusiness?.lat) {
  //     amapIns.setZoomAndCenter(BUSINESS_FIT_ZOOM, [curSelectedBusiness?.lng, curSelectedBusiness?.lat], true);
  //   }
  // }, [curSelectedBusiness?.planClusterId]);
  ...
  return (
    <div className={cs(styles.container, networkMapContainer)}>
      ...
      <MapCon
        ...
        setDetailData={setDetailData}
        detailData={detailData}
        planClusterId={curSelectedBusiness?.planClusterId}
        city={city}
