...
const AddressMarker:FC<any> = ({
  ...
  // 当选中市场容量（商区）时，显示商区内部商圈的围栏--被写进绘制过程中-因为这样可能导致先后执行，先显示在隐藏
  // useEffect(() => {
  //   //  如果展示商区围栏，则不展示具体点位的围栏
  //   if (isShowBusinessDistrict) {
  //     polygonMarkerRef.current.map((marker) => {
  ...
  //     }
  //   }
  // }, [selectedBusinessDistrict, isShowBusinessDistrict, businessAreaMap, polygonListData]);
  ...
  // 选中绘制商圈，不展示addressMarkerRef --- 被写进绘制过程中-因为一移动就根据可视范围拿数据
  // useEffect(() => {
  //   if (isShape) {
  //     addressMarkerRef.current.map((item) => {
  //       item.hide();
...
const Point:FC<any> = ({
  ...
  const showStatus = () => {
    if (node?.isBranchCompanyEliminate) { // 已剔除 无后缀
      return null;
    }
