...
// import { markerType } from '../../ts-config';
...
const BusinessDistrict = ({
  ...
  // const businessDistrictMarkerRef = useRef<any>([]);
  ...
  const createBusinessDistrictMarker = () => {
    ...
    if (isShowBusinessDistrict && !(listData.length >= maxAddressMarkerCount)) {
  ...
  const jumpBusinessArea = () => {
    if (selectedBusinessDistrict.businessAreaId) {
      areaData.map((item) => {
        if (item.businessAreaId === selectedBusinessDistrict.businessAreaId) {
          setSelectedBusinessDistrict({
            ...item,
            visible: true
          });
  ...
  useEffect(() => {
    if (!amapIns) return;
    if (!(listData.length >= maxAddressMarkerCount) && isShowBusinessDistrict) {
  ...
  }, [amapIns, areaData, isShowBusinessDistrict]);
