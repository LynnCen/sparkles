...
import { addToPlan, cancelThePlan } from '@/common/api/networkplan'; // 无用，实际上不会请求
...
// TODO 详情需要拷贝一份出来
import NetworkPlanLeftDetail from './NetworkPlanLeftDetail';
...
const List: FC<any> = ({
  ...
  const dynamicHeight = useMemo(() => {
    // 底部footer高 56 选中行高40
    // if (selectedRowKeys.length > 0) return mainHeight - 56 - 40;
  ...
  const methods = useMethods({
    ...
    handleCell(record: any) {
      ...
      amapIns?.setZoomAndCenter(BUSINESS_FIT_ZOOM, [record?.lng, record?.lat], false, 300);
    ...
    <>
      {
        detailData?.visible ? <div className={styles.networkDetail}>
          ...
          <Spin
            ...
            { (isArray(data) && data.length) ? <ListCom>
              {/* 虚拟列表 */}
              <VirtualList
                data={data}
            ...
            </ListCom> : <V2Empty className='mt-100'/>}
          </Spin>
