...
function parser(text) {
    // 处理 linkStr 连接前面添加空格，
    text = text.replace(/(http|https)/gim, ` $1`);
    ...
    var emojiEG = /(\<img class="emoji" draggable="false" alt=\"[^\">]*?\" src=\"[^\"]*?\"\/>)|(\<a href=\"[^\"]*?\"  target\=\"\_blank\"\ >[^\<]*?\<\/a\>)/gm;
    emoji = emoji.replace(
        /src="https:\/\/twemoji\.maxcdn\.com\/v\/12\.1\.6\//g,
        'src="assets/twemoji/'
    );
    return emoji
        .replace(emojiEG, ($0) => {
        ...
        .split(/\$\#\$/gm)
        .map((item, index) => {
            if (emojiEG.test(item)) {
                return <span key={index} dangerouslySetInnerHTML={{ __html: item }} />;
            }
            return item
                .replace(/\<div\>\<br\/?\>\<\/div\>/gm, "\n")
                .replace(/\<br\/?\>/gm, "\n")
                .replace(/<\/?div>/gm, "");
        });
}

function parser1(text) {
    var decodeText = anchorme(text, {
        attributes: [
            {
                // attribute name
    ...
    //         return;
    //     }
    //     text = decodeText = text.split(`${e}`).join(`<a target="_blank" className="${className}"></a>`);
    // });
...
function normalize(text = "") {
    var matchs = text.match(/<span className="emoji emoji[0-9a-fA-F]+"><\/span>/g) || [];
    var decodeText = text;
...
export { emoji, parser, normalize, parser1 };
