 ...
 * @Description 添加字段弹窗页
 * 该组件是从
 *《src/views/location/pages/tenantdetail/components/StoreTemplateConfig/FieldTree/index.tsx》
 * 拷贝出来作为公共业务组件的
 * 注意这个组件有多个地方引用，修改的时候需要注意
 */
...
import FieldFilters from './components/Filter';

const FieldTreeDraw: FC<any> = ({
  templateId,
  onSearch,
  ...
  const defaultRequires = ['franchiseeName', 'franchiseeStatus', 'uniqueIDFranchisee'];
  const columns = [
    {
      key: 'name',
      title: '分类名称',
        ...
        propertyConfigRequestList: [
          {
            ...
            required: defaultRequires.includes(record.identification) ? 1 : null
          },
        ],
      };
