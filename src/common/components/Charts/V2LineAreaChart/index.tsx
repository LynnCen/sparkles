...
export interface V2LineAreaChartProps {
  /**
   * @description 图表标题
   */
  title?: string;
...
/**
* @description 便捷文档地址
* @see https://reactmobile.lanhanba.net/charts/v2-line-area-chart
*/

const V2LineAreaChart: React.FC<V2LineAreaChartProps> = (props) => {
  const {
    title,
    theme = 'blue',
    width,
        ...
        setOptions({
          ...
          series: seriesData.map((item, index) => {
            return {
              ...seriesBaseConfig,
              ...item,
              areaStyle: seriesAreaStyleConfig(index),
              label: {
  ...
  return (
    <div className={cs(styles.V2LineAreaChartWrap, wrapperClassName)} style={{ width, height }}>
      <div ref={chartRef} className={styles.V2LineAreaChart}></div>
    </div>
  );
};

export default V2LineAreaChart;
