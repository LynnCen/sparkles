...
// 访问场地项目模糊搜索
const VisitPlaceProject: React.FC<any> = forwardRef(({
  name,
  extraParams = {}, // 额外的传参
  label,
  ...
  const loadData = async (keyword?: string) => {
    ...
    const data = await post('/brand/place/queryList', params, {
      needCancel: false,
      proxyApi: '/zhizu-api'
    });
    setData(data);
  ...
  const customOptionItem = (user: Record<string, any>) => {
    const { placeName, contact, placeId } = user;
    return (
      <>
        <span key={placeId}>{placeName || '-'}{contact?.userName ? `-${contact?.userName}` : ''}</span>
      </>
    );
  };
...
export default VisitPlaceProject;
