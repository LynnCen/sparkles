...
const LogoCrop = forwardRef<Cropper, LogoCropProps>((props, ref) => {
  ...
  const cb = useRef<
    Pick<
      LogoCropProps,
      'onModalOk' | 'onModalCancel' | 'onUploadFail'
    >
  >({});
  ...
  const getComponent = (titleOfModal) => (
    <AntModal
      open={visible}
      wrapClassName={cs(`${PREFIX}-modal ${modalClassName || ''}`, styles.logoCrop)}
      title={titleOfModal}
      onOk={onOk}
      onCancel={onCancel}
      maskClosable={false}
      ...
      footer={<div>
        <Upload
          ...
          className={styles.footerUpload}
          onChange={onLogoChange}
        >
          <Button type='text' className={cs('left c-006', styles.resetBtn)} disabled={!image}>重新上传</Button>
        </Upload>
        <Button onClick={onCancel}>取消</Button>
        <Button type='primary' className={styles.okBtnDisabled} onClick={onOk} disabled={!image}>确定</Button>
      </div>}
      {...modalProps}
    >
      ...
      <div
        ...
        <div>
          ...
          >
            <TemplateUpload
              classNames={styles.templateUpload}
              text='点击上传LOGO' />
          </Upload>
