...
import { Cascader, Col, Popover, Row, Form } from 'antd';
...
const Module2: FC<any> = ({
  ...
  const probaValue:any = Form.useWatch('proba', form);
  ...
  // };
  useEffect(() => {
    if (!isArray(probaValue)) return;
    setProbaVal(probaValue);
  }, [probaValue]);
  return (
    <Row gutter={16}>
      <Col span={12}>
      ...
      <Col span={12}>
        <V2FormSelect
          label='模型商圈类型'
          name='type'
          options={refactorSelection(selections.clusterType)}/>
      </Col>
    </Row>
