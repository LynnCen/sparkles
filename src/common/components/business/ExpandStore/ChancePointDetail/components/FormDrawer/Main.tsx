...
import RecoupCostPeriod from './RecoupCostPeriod';
...
const Main: FC<any> = ({
  ...
  const topHeight: number = (topRef?.current?.getBoundingClientRect()?.height || 0) + 24;
  ...
  useEffect(() => {
    ...
    return () => selfRef.current && (selfRef.current.removeEventListener('scroll', handleScroll));
  }, []);
  const {
    onValuesChange,
    addressChange,
    ...
    /**
     * 获取tab内容的dom元素
     * @param el 对应content的dom元素
     * @param index 对应一级菜单索引
     * @param id 对应一级菜单的id (key)
     */
    getTabContentRefs: (el: any, index: number, id: number) => {
      tabContentRefs.current[index] = {
        el,
        id
      };
    },
    handleScroll: debounce(() => {
      // 所有content此时距离顶部的距离，需要减去(吸顶部分的高度 + 抽屉上边距）
      const offsets = tabContentRefs.current.map((refItem) => refItem?.el?.getBoundingClientRect()?.top - topHeight);
      // 当前tab内容所处的索引
      ...
      const targetIndex = tabContentRefs.current.findIndex((item) => item.id === +tabActiveRef.current);
      const targetOffset = targetIndex !== -1 ? offsets[targetIndex] : 0;
      if (activeIndex !== -1 && tabs[activeIndex]) {
        // 滚动条已经滚动到底，但是可视区域顶部还在其他tab内容区域下（最后的几个tab可能内容很少，出现在一屏）
        if (isBottomOut() && targetOffset > 0) {
          setTabActive(tabActiveRef.current);
          return;
        };
        const target = tabContentRefs.current[activeIndex];
        target && target.id && setTabActive && setTabActive(`${target.id}`);
      }
    }, 50),
    isBottomOut: () => {
      const scrollTop = selfRef.current.scrollTop;
      const scrollHeight = selfRef.current.scrollHeight;
      const clientHeight = selfRef.current.clientHeight || selfRef.current.innerHeight;
            ...
            formData?.propertyGroupVOList ? <>
              {
                ...
                >
                  {
                    (isLongList || groupData.id === +active) ? <DynamicComponent
                      form={form}
