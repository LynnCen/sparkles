...
import { deepCopy, debounce, isNotEmpty, concatObjArray } from '@lhb/func';
...
const { Option } = Select;
const Fuzzy: React.FC<FuzzyProps & SelectProps> = forwardRef(({
  ...
  useImperativeHandle(ref, () => ({
    ...
    reload: () => loadDataHandle(keywordStr)
  }));
  ...
  const loadDataHandle = async (keyword: string, fetchId?: number) => {
    const data = await loadData(isNotEmpty(keyword) ? keyword.trim() : keyword);
  ...
  // 提供给父级，根据传入 id/ids 获取选中项；父级使用 getItem: (data) => fuzzyRef.current.getItem(data)
  const getItem = (data) => {
