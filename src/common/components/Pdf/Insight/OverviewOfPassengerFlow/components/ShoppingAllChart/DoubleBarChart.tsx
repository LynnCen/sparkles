...
const DoubleBarChart: React.FC<any> = ({
  ...
  const xAxisData = useMemo(() => {
    return getKeysFromObjectArray(data?.find((item) => item.type === 'indoorCount').data, 'name');
  }, [data]);

  const shoppingCountData = useMemo(() => {
    return data?.find((item) => item.type === 'shoppingCount').data;
  }, [data]);
